...
# A comprehensive tech and industry taxonomy of common terms
TAXONOMY = {
    "Technical Skills": [
...
def clean_text(text: str) -> str:
    ...
    text = text.lower()
    text = re.sub(r'[^\w\s#+.-]', ' ', text)  # Keep special chars like C++, C#, .NET
    return re.sub(r'\s+', ' ', text).strip()

def extract_keywords_from_text(text: str) -> Set[str]:
    """Extracts valid candidate keywords based on our taxonomy and JD content."""
    cleaned = clean_text(text)
    words = set(cleaned.split(' '))
    
    extracted = set()
    # 1. Match against Taxonomy
    ...
    # 2. Extract potential capitalized nouns / tech keywords (fallback heuristic)
    for word in words:
        if len(word) > 2 and word not in STOP_WORDS and not word.isdigit():
            # If word is in JD and looks like a skill (e.g. length > 3, alphanumeric)
...
def calculate_ats_metrics(resume_text: str, jd_text: str) -> Dict[str, Any]:
    """
    Computes overlap, missing keywords, and section compatibility.
    """
